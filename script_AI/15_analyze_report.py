...
# you can add a high-level instruction here
SYSTEM_PROMPT = (
    "You are a fact-checking assistant."
    "If you are not confident in an answer, respond: ‘I don’t have enough information to answer that.’"
    "If confident, give your answer with a short justification."
)

# Fixed user-side prologue that is always sent to the model.
BASE_PROLOGUE = (
    "Please review the below DRAFT SITREP (situation report) and "
...
def parse_cli() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Analyze SITREP file using LM Studio."
    )
    parser.add_argument(
        "report_path",
        help="Path to the SITREP file to analyze.",
    )
    parser.add_argument(
        "-i",
        "--instruction",
...
def main() -> None:
    ...
    report_content = read_file_content(args.report_path)
    print(f"[+] Read report content from: {args.report_path}\n")
    parts.append("--- START DRAFT SITREP ---\n")
